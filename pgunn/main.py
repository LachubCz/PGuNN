...
def scoring(task, scores, episodes_numbers, eps, test_type="always", test_freq=None, save_freq=None, refresh_freq=None):
    """
    method for working around with different variables after game reached finite state
    """
...
def get_args():
    ...
    parser.add_argument("-env", action="store", dest="environment", required=True,
                        choices=["CartPole-v0", "CartPole-v1", "MountainCar-v0", "Acrobot-v1"],
...
def vect_game(task, args):
    """
    method for playing vector games
    """
    scores = []
    episodes_numbers = []
        ...
        for t in range(task.max_steps):
            ...
            if done:
                scores, episodes = scoring(task, scores, episodes_numbers, eps, 
                    test_type="always", test_freq=1, save_freq=50, refresh_freq=None)
...
def main():
    ...
    if args.model is not None:
        ...
        task.agent.current_epsilon = 1 #initial epsilon
